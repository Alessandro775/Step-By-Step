  ...
  const fetchCronologia = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:3000/api/family-cronologia",
        {
          method: "GET",
          headers: {
      ...
      setStudenteInfo(data.studente);
      setCronologia(data.cronologia);

      // Prepara i dati per i grafici
      const chartDataPreparata = prepareChartData(data.cronologia);
      const errorsAttemptsDataPreparata = prepareErrorsAttemptsData(data.cronologia);
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerCenter}>
          <h2>
            {studenteInfo 
              ? `Cronologia di ${studenteInfo.nome} ${studenteInfo.cognome}` 
              : 'Cronologia dello Studente'
            }
          </h2>
          <p>Visualizza i progressi di tuo figlio negli esercizi</p>
        </div>
        ...
        <div className={styles.contentSection}>
          {cronologia.length === 0 ? (
            <div className={styles.emptyState}>
              <p>
                {studenteInfo 
                  ? `${studenteInfo.nome} non ha ancora completato nessun esercizio` 
                  : 'Nessun esercizio completato'
                }
              </p>
              <p>
                I progressi di tuo figlio appariranno qui una volta completati
                    ...
                    <tr key={record.idRisultato || index}>
                      ...
                      <td>
                        {record.data_completamento
                          ? new Date(record.data_completamento).toLocaleString(
                              "it-IT"
                            )
                          : "N/D"}
                      </td>
